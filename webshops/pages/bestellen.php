...
if(isset($_post["submit"])) {
    ...
    $sql = "INSERT INTO weborder (ID, klant_ID, datum)
    values (?,?,?)";
    ...
    for($x=0; $x < $_POST['lus']; $x++){
        ...
        $sql = "INSERT INTO item (ID, weborder_ID, klant_ID, album_ID, 
        prijs_eenheid, aantal) values (?,?,?,?,?,?)";
        $stmt = $verbinding->prepare($sql);
        $data = array(NULL, $weborder_id $_SESSION["USER_ID"],
        $album_id, $prijs_eenheid, $aantal);

        try{
            $stmt->is_execute($data);
            echo "<script>alert('Kon geen item opslaan');</script>";
        }catch(PDOException $e) {
            echo $e->getMessage();
            echo "<script>alert('Kon geen item opslaan');</script>";
        }
        "<script>location.hreg='index.php?page=webshop';</script>"
    }
}
?>